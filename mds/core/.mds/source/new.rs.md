...
| internal | crate::model | NewOptions | - | - | [model.rs.md#source](model.rs.md#source) |
...
pub(crate) fn run_new(
    ...
    let is_overview = name == "overview.md" || name.ends_with("/overview.md");
    let is_module_root = is_module_root_markdown_name(name);

    if matches!(doc_kind, DocKind::Source) && !is_overview && descriptor.is_none() {
        return Err(format!(
    ...
    let content = if is_overview {
        generate_overview_template(&feature_name, &labels, doc_kind)
    } else if is_module_root {
        generate_module_root_template(&feature_name, &labels)
...
````rs
fn is_module_root_markdown_name(name: &str) -> bool {
    matches!(
        name.rsplit('/').next(),
...
fn generate_overview_template(
    ...
    let architecture = match doc_kind {
        DocKind::Source => "Markdown files in this directory describe package and source hierarchy rules.",
...
fn generate_module_root_template(feature_name: &str, labels: &HashMap<String, String>) -> String {
    let l_purpose = label(labels, "purpose", "Purpose");
    let l_exports = label(labels, "exports", "Exports");
    let l_imports = label(labels, "imports", "Imports");
    let l_name = label(labels, "name", "Name");
    let l_visibility = label(labels, "visibility", "Visibility");
...
fn generate_impl_template(
    ...
    format!(
        "<!-- Generated by mds new. -->\n\
         ...
         | {l_name} | {l_visibility} | {l_summary} |\n\
         | --- | --- | --- |\n\
         | {feature_name} | public | Shared entrypoint for other modules. |\n\
         \n\
         ##### {feature_name}\n\
         \n\
         Describe why this shared definition exists and who should reference it.\n\
         \n\
         ## {l_imports}\n\
         \n\
         | {l_from} | {l_target} | {l_symbols} | {l_via} | {l_summary} | {l_reference} |\n\
...
fn generate_test_template(feature_name: &str, labels: &HashMap<String, String>) -> String {
    ...
    format!(
        "<!-- Generated by mds new. -->\n\
         ...
         | {l_from} | {l_target} | {l_symbols} | {l_via} | {l_summary} | {l_reference} |\n\
         | --- | --- | --- | --- | --- | --- |\n\
         | internal | ./{feature_name} | {feature_name} | - | Function under test | [./{feature_name}.ts.md#{feature_name}](./{feature_name}.ts.md#{feature_name}) |\n\
         \n\
         ## {l_cases}\n\
         \n\
         - Describe the expected behavior.\n\
