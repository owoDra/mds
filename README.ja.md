...
インストーラーは platform 別の GitHub Releases archive を取得し、`mds` と `mds-lsp` の両方をインストールします。Marketplace 版の VSCode 拡張には対応する `mds-lsp` バイナリが同封されるため、VSCode 利用時は通常 LSP を別途インストールする必要はありません。
