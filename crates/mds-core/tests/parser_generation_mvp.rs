...
fn init_generates_selected_ai_agent_kit_and_project_skeleton() {
    ...
    assert!(temp.path().join(".opencode/instructions.md").exists());
    assert!(temp.path().join(".opencode/commands/mds-check.md").exists());
    assert!(!temp.path().join(".claude/skills/mds/SKILL.md").exists());
    let claude = fs::read_to_string(temp.path().join("CLAUDE.md")).unwrap();
    assert!(claude.contains("Generated by mds init"));
    assert!(claude.contains("mds check"));
...
fn init_refuses_nonmanaged_overwrite_without_force() {
    let temp = TestDir::new();
    fs::write(temp.path().join("CLAUDE.md"), "manual\n").unwrap();
    let result = execute(CliRequest {
        cwd: temp.path().to_path_buf(),
        package: None,
        verbose: false,
