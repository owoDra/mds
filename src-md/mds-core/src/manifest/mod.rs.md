...
Migrated implementation source for `src/manifest/mod.rs`.

## Contract

- Preserve the behavior of the pre-migration Rust source.
- This file is synchronized into `.build/rust/mds-core/src/manifest/mod.rs`.
...
pub(crate) fn validate_manifest(package: &Package, state: &mut RunState) {
    let path = package.root.join(".build/mds/manifest.toml");
    if !path.exists() {
        return;
    }
...
pub(crate) fn plan_manifest(
    ...
    GeneratedFile {
        path: package.root.join(".build/mds/manifest.toml"),
        content,
