...
Migrated implementation source for `src/markdown/mod.rs`.

## Contract

- Preserve the behavior of the pre-migration Rust source.
- This file is synchronized into `.build/rust/mds-core/src/markdown/mod.rs`.
...
pub fn extract_all_code_blocks(text: &str) -> String {
    ...
    for line in text.lines() {
        if let Some(marker_len) = backtick_fence_len(line) {
            if let Some(open_len) = fence_len {
                if marker_len < open_len {
...
fn backtick_fence_len(line: &str) -> Option<usize> {
