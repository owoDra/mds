...
fn builds_three_language_fixture() {
    ...
    assert!(dry_run.stdout.contains(".build/mds/manifest.toml"));
    ...
    assert!(temp.path().join("pkg/.build/mds/manifest.toml").exists());
}
...
fn rejects_broken_manifest_before_building() {
    ...
    fs::create_dir_all(temp.path().join("pkg/.build/mds")).unwrap();
    fs::write(
        temp.path().join("pkg/.build/mds/manifest.toml"),
        "not manifest\n",
    )
    .unwrap();
