...
mds は npm、Cargo、Python / uvx、native binary の全配布経路について、公開前に artifact、checksum、署名、SBOM、provenance、install smoke test を確認できること。

## 根拠

mds は多言語・多エコシステムに配布される CLI であり、利用者が取得した binary / wrapper / package の完全性、互換性、実行可能性を確認できる必要があるため。

## 対象範囲

- npm package、Cargo crate、Python package / uvx entrypoint、native binary の公開前検証
- OS / architecture 別 artifact の一覧、checksum、署名、SBOM、provenance / attestations
- install 後 smoke test と wrapper 経由の exit code / stdout / stderr 確認
...
- 全配布経路で公開前 smoke test が実行できる
- artifact ごとに checksum、署名、SBOM、provenance が紐づく
- wrapper と native CLI の version / compatibility mismatch を公開前に検出できる
