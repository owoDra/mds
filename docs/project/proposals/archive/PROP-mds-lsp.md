...
archived: Self-hosting Removal and Authoring V2 の Phase 00 で参照優先度を下げた。repository implementation planning は `docs/plan/index.md` と `docs/plan/phase-07-lsp-bridge.md` を優先し、この proposal は旧 `src-md` / `.build` self-hosting 前提の historical context として保持する。
...
- パフォーマンスに優れる
- 生成された Cargo ワークスペースでは `.build/rust/mds/lsp/` として扱い、正本は `src-md/mds/lsp/` に置く

### アーキテクチャ

```text
src-md/
  mds/core/       # パーサー、検証ロジックの正本
  mds/lsp/        # LSP サーバーの正本
    src/
      main.rs      # LSP サーバー起動
...
- `.build/rust/mds-lsp` のバイナリを `bundledServerPath` で同梱
...
1. `src-md/mds/lsp/` の正本を追加し、`.build/rust/mds/lsp/` に生成
