...
- implementation md の必須セクションは H2 固定とし、各セクション内の補助見出しは H3-H4 まで許容する。
...
- implementation md は package 内の `markdown_root` 配下に分離して置き、既定の `markdown_root` は `src-md` とする。
...
- implementation md の必須セクションが H2 以外で書かれている場合は構造エラーにする。
- implementation md の補助見出しが H5 以降に深くなる場合は構造エラーにする。
...
- `src-md` 配下の implementation md から生成対象の相対 path を解決できることを確認する。
