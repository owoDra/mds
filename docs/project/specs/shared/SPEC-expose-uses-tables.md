...
- `Uses.Expose` で複数名や import 表現拡張を使う場合は CSV tokens としてカンマ区切りで書く。
- `Uses.Expose` token は named import / use の `A`、alias の `A as B`、default import の `default: Foo`、namespace import の `* as ns` を扱う。
- alias 表記は `A as B` のみを正式表記とする。
- 同一 `Uses.Expose` cell 内の組み合わせは TypeScript import 準拠とし、named 単独、default + named、namespace 単独を許可し、default + namespace は許可しない。
...
- 同一 `Uses.Expose` cell 内で文法不正または不許可の default / alias / namespace の組み合わせは検査エラーにする。
- 共通 parser で受け入れた default / namespace が Python / Rust など対象 adapter で変換できない場合は adapter 診断にする。
...
- CSV tokens、default + named、namespace 単独、不正な default + namespace、adapter 非対応 import 表現を fixture で確認する。
