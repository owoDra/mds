...
- Post-MVP の `mds.config.toml` は実装対象の quality 操作、doctor、package sync、exclude path、label override の設定を扱う。
- quality 操作設定は adapter ごとの linter、fixer、test runner の有効 / 無効と追加引数を扱う。
- adapter ごとの追加引数は adapter 別 table に置く。
- exclude path は package root 相対 glob とし、Markdown 正本、生成物、toolchain 一時ファイルの対象除外に使う。
- label override は canonical key に紐づけ、`Uses`、`Expose`、必須セクションの意味を変更しない。
- package sync hook は `[package_sync] hook_enabled = true` で明示有効化し、`hook_command` 未指定時の既定 command は `mds package sync --check` とする。
