...
- 他ファイルから参照される公開定義は、table metadata だけでなく H5 見出しと本文説明を持つ。
- source md は実装前の spec state でも `Purpose`、`Contract`、`Exports`、`Cases` により設計・仕様を読める状態にする。
...
- 可読性を損なう機械専用メタデータは採用前に package overview、pattern、または ADR で正本化する。
