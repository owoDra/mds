...
- mds の Rust crate では、1 ファイル module は `src/<name>.rs` へ置く。submodule や template など追加ファイルを持つ module だけ `src/<name>/mod.rs` を使う。
...
- `.github/script/sync-build.sh`、`.build/rust` での `cargo fmt --check` と `cargo test` を Rust 実装変更の最小検証にする。
