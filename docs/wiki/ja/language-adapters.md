...
mds の中核処理は、Markdown の読み取り、構造検査、生成計画を担当します。TypeScript、Python、Rust には組み込みのアダプターがあり、それ以外の言語（Go、Java、Swift など）は汎用アダプターで自動対応します。

## 対応言語

mds は任意の `*.{lang}.md` ファイルを認識します。例: `greet.go.md`、`utils.java.md`、`handler.swift.md`

組み込みアダプターがある言語では、より適切な出力パス命名やインポート生成が行われます。汎用アダプターでは基本的な生成が行われます。
...
これらのツールは、利用する機能に応じて選択します。`mds init` で選んだ内容は `mds.config.toml` の `[quality.*]` に明示され、未選択のツールは暗黙には実行されません。

## その他の言語

Go、Java、Swift など、組み込みアダプターがない言語でも `mds.config.toml` で品質ツールを設定できます:
...
| 種別 | 生成先の例（`.go.md` の場合） |
| --- | --- |
| `Source` | `src/foo/bar.go` |
| `Types` | `src/foo/bar.types.go` |
| `Test` | `src/foo/bar.test.go` |
