...
└── tests/
    ├── capabilities.rs      # capability 関連テスト
    ├── capabilities_test.rs # capability の統合テスト
    └── diagnostics_test.rs  # 診断の統合テスト
```
...
リリース用バイナリも同じ root Cargo workspace から作成します:
...
共有の `mds-core` API も変更した場合は、repository root で `cargo test --workspace` も再実行してください。
...
`mds/*` 配下の first-party package では、checked-in Rust source を直接編集し、repository root の Cargo を通常の build / test / lint 入口として使ってください。
...
1. `cargo build -p mds-lsp` で debug バイナリをビルド
2. `mds.lsp.path` に debug ビルドのパスを設定（例: `target/debug/mds-lsp`）
3. VSCode 拡張を再起動
4. Rust 側のデバッグは `tracing` ログで行うか、別途 `rust-lldb`/`rust-gdb` を使用

## 新しい capability の追加手順

### 1. capability モジュールの作成

`mds/lsp/src/capabilities/` に新しい Rust module を作成します。
...
`mds/lsp/tests/capabilities_test.rs` または新しいテストファイルにテストを追加:
