...
- `.github/skills/docs-update-spec/references/spec.template.md`
- `.github/skills/docs-update-spec/references/best-practices.md`
- 関連 requirement / ADR / validation / pattern / architecture

## やること

1. 必要なら `ask_user` で対象 requirement、主要な入出力、状態、エラー、横断ルールを確認する
2. 共有仕様か subproject 固有仕様かを判断する
3. 既存 spec と重複、矛盾がないか調べる
4. `shared/` または `<subproject>/` に `SPEC-<category>-<short-title>.md` を作成または更新する
5. `.github/skills/_shared/document-reference-rules.md` に従い、配置先に対応する `index.md` を必ず更新する
